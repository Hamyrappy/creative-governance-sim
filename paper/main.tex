...
\item \textbf{A decomposition of what a stale rule costs, computed before any agent is run}
(\cref{sec:headroom,sec:decomposition}). Three references calibrated in one shared vocabulary---the
pre-break optimum held fixed, the best \emph{fixed} rule in hindsight, and the best clairvoyant
\emph{switch}---factor staleness exactly into an adaptation headroom and a robustness headroom, and
place any controller on a normalized scale between the last two. Because it is computed first, it
makes the severity of a designed shock an auditable choice rather than a researcher degree of
freedom.
...
intervention, non-monotone in each, and \emph{scarce}: only $\libOverEleven{}$ of $\libWorlds{}$
worlds clear $1.1\times$. We report the near-null worlds rather than discarding them, and retain two
...
\Cref{tab:library} is the claim in its general form. Across \libWorlds{} worlds only
\libOverEleven{} clear $1.1\times$ adaptation headroom and \libOverTwelve{} clears $1.2\times$.
Scarcity is the rule, not an artifact of one pathogen.
...
Because a corner optimum is invariant to any regime, we checked whether that zero-lower-bound
intercept is an artifact of the search grid: extending the grid to admit intercepts \emph{below} the
bound moves adaptation headroom only from $1.359$ to $1.362$. Committing below the zero lower bound
buys essentially nothing, which is the economically correct answer, so the corner is a real
constraint of the instrument rather than an edge of our vocabulary.
...
\emph{not} to withdraw but to \emph{substitute}---vaccination is held at its maximum throughout while
...
\subsection{A prior strong enough to override the mandate}
\label{sec:priors}

The monetary world produces the sharpest single failure we have measured, and it is a failure of a
kind the epidemic cannot elicit, because it depends on the agent having opinions about the domain
before it arrives.
...
$R \approx 3.0$: three times the cost of the non-adaptive ceiling it was supposed to beat. It is not
failing to adapt---it is adapting energetically in the wrong direction.

The mechanism is visible in the last two columns. The regent attains the \emph{lowest mandate burden
of any arm}, $608$ against the clairvoyant's $807$. It stabilizes inflation and output better than the
policy that beats it by a factor of two. It simply pays $13{,}131$ for the privilege, against $4{,}259$,
because it holds the policy rate at $9.51$ after a break that has severed the rate's connection to
the economy---higher than the frozen rule it was meant to improve on, and six times the clairvoyant's
$1.53$. In the mandate's own currency it trades $199$ units of burden for $2{,}218$ units of cost.

What it writes is not incompetent. Across $400$ decisions it emits $123$ distinct policies, nearly all
recognizable Taylor rules---\texttt{r\_star + 2.0 * (inflation - inflation\_target) + 2.0 *
output\_gap} and its variants---with feedback gains of $1.5$ to $3.0$. The calibrated optimum, by
contrast, sets that gain to \emph{zero} and sits at a constant. The regent is not producing noise; it
is producing textbook monetary policy, competently, into a world where textbook monetary policy is
the wrong answer.

The part that makes this more than an anecdote about a hard task is that the mandate says so, in
plain language, in every prompt. It states that the rate's cost accrues on the level set, every step,
``\emph{and is charged whether or not the rate reaches the economy: a restrictive stance is
politically and financially expensive even when it changes nobody's behaviour}.'' That is a direct,
correct, unambiguous warning about precisely the trap the world contains. The regent read it on all
$400$ decisions and leaned harder anyway.

We take two things from this. First, as a methodological point, it rules out the interpretation that
first occurred to us: this is not the objective-communication failure we had already found and fixed
elsewhere in this project, because the objective is communicated and it is communicated well.
Second, and more substantively: \emph{an explicit mandate is not sufficient protection against a
strong parametric prior}. A language-model authority arrives already knowing what central banks do,
and that knowledge is not neutral background---it is an active commitment that survived an
instruction written specifically to displace it. Domain familiarity, usually treated as an asset when
selecting a model for a governance task, is here a liability, and one that grows with the model's
competence in the domain.

\begin{takeaway}
The regent's failure was not incoherence but fluency: it applied the correct textbook response to a
world whose textbook had stopped applying, while holding a written statement of the trap in context.
\end{takeaway}
...
\midrule
no harness & $0.850$ & $14.4$ \\
failure trace only & $0.855$ & $14.5$ \\
outcome feedback only & $0.774$ & $8.2$ \\
...
memory-equipped agent restores most of what memory removed. That interaction is mirrored in the loss
factorial, where $\text{outcome} \times \text{memory}$ is $-0.530$---the two components partially
cancel, for a reason the churn measurement makes concrete. Precedent tells the authority what it did;
monitoring tells it that what it did is no longer working. The first alone produces lock-in, and the
second is what breaks it.
...
Under the permissive convention the outcome main effect rises from $+0.190$ to $+0.230$ while memory
and the interaction are unchanged to two decimals; we report the conservative convention throughout
...
\emph{Bottom}: full-horizon loss on the same ordering, against the three calibrated references. Note
that no arm reaches the best fixed rule, so the figure is not a story about one component winning ---
it is about which component moved the institution's behaviour, and in which direction. We
deliberately do not fit a line between the two panels: the two-way fixed-effects correlation between
churn and loss is $-0.11$, so a fitted line would assert a within-condition relationship the data
does not support.}
\label{fig:lockin}
\end{figure}
...
ignores the epidemic state entirely---and adding episodic memory left $378$ of $400$ unchanged. Its
six arms agreed to four decimal places. The large model spread its decisions across many policies
(top-policy share $50\%$) and shifted them substantially when a channel was added (total-variation
distance $0.38$ against $0.003$).
...
in a loss table. Our analysis now gates on the per-arm rate of decisions that produced no parseable
action, refuses to interpret anything above $2\%$, and the reported factorial is a re-run with the
budget raised until that rate is zero everywhere.
...
normalized regret; the shaded band is the minimum detectable effect around the no-harness arm. It is
wide enough to swallow the entire distance from the arms to the non-adaptive ceiling, which is the
honest reading of this experiment: the configurations are indistinguishable from one another
\emph{and} the design could not have separated them unless a component were worth roughly $\MDEFraction\%$ of
everything adaptation is worth here. Reporting the bar ordering without the band would invite a
...
\paragraph{A narrow band, found by search.} Of the four regimes we calibrated, exactly one has
adaptation headroom above $1.05$. We reached it by measuring rather than by tuning---the severity
surface was computed before any treatment arm ran, and the operating point sits inside the usable
band rather than at its maximum---but a reader should still weigh that the headline regime is the
survivor of a search, and that the three that did not survive are reported alongside it precisely so
...
the protocol level (budget matching, factorial structure, the five validity gates), and do not vary
prompt phrasing systematically---which, given that a single added paragraph about the objective
...
substantive error in this paper was found by an internal check rather than by review: a metric that
rewarded passivity, three channels that carried no signal, output truncation correlated with the
treatment, reference anchors superseded by a recalibration, a clairvoyant reference beatable by a
fixed policy, a confounded severity cell, and a mandate the agent was never given. Each produced a
clean, plausible, wrong number. We have no reason to believe the list is complete, and we would
rather say so than imply that the seventh check was the last one needed.
