...
\paragraph{``Recall itself'' still covers two claims. A fourth arm separates them.}
The sentence above hides an ambiguity with opposite design consequences. Under \emph{anchoring}, any
concrete exemplar of a policy pulls the agent toward it, and precedent is unusable across a break in
any form. Under \emph{commitment}, what binds is specifically being shown \emph{one's own} prior
decisions, and precedent remains usable provided the agent did not author it. Presentation repairs
cannot tell these apart, because all three showed the agent its own history.

We therefore ran a fourth arm that holds retrieval, ranking, phrasing and episode count fixed and
changes only \emph{whose} episodes are in the bank. The agent is given eight episodes recorded from a
different authority governing a different realization of the same world, and never sees its own. The
prediction was registered before the arm ran: under commitment, churn rises materially toward the
no-memory arm's $0.847$; under anchoring it stays near $0.082$.
...
\textbf{another authority's} & $\mathbf{0.632}$ & $\mathbf{25.616}$ & $\mathbf{0.995}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Commitment. Foreign precedent reproduces only $27\%$ of own-precedent's suppression; the
authorship residual is the other $73\%$, and all three pairwise contrasts survive Holm correction
($p_{\text{adj}} \le 0.009$, $n=10$ paired seeds). On loss, foreign precedent removes the entire harm
own precedent causes ($\Delta = -1.121$, CI $[-2.906, -0.351]$, $p = 0.025$) and is the only memory
arm that reaches the best fixed rule. It does \emph{not} beat the no-memory arm
($\Delta = -0.244$, $p = 0.61$), so the claim is that foreign precedent is harmless where own
precedent is harmful, not that it helps.

Three alternative readings were tested and all fail. \textbf{(i)~Retrieval similarity.} Own memory
draws from the agent's own trajectory, so it might simply return closer precedent. Measured directly
with the same component and a scripted policy, foreign precedent is retrieved $7.9\times$
\emph{closer}, not further---the endemic equilibrium makes different realizations traverse nearly the
same states. The confound runs backwards. \textbf{(ii)~Bank composition.} The bank the arm actually
received is eight episodes from a \emph{single} donor run, so it is already a one-authority design
rather than a pooled crowd. \textbf{(iii)~Bank repetitiveness.} The donor's eight episodes are eight
different laws while own memory fills with the agent's converged history, so perhaps repetition is
what anchors---which predicts late-onset lock-in. \Cref{fig:lockin-onset} shows the onset is
immediate: with a bank of \emph{one} episode the own-memory arm already revises at
$\lockinOnsetOwn$ against $\lockinOnsetBare$ with no memory, and by two episodes it is zero and stays
zero for ten consecutive decisions. A one-episode bank cannot be repetitive.
...
foreign arm responds ($\lockinForeignPre \to \lockinForeignPost$, $\lockinForeignDelta$,
$p_{\text{Holm}} = \lockinForeignP$). The no-memory arm does not respond
($\lockinBareDelta$, $p = \lockinBareP$) because it revises constantly regardless, and the own-memory
arm does not respond ($\lockinOwnDelta$, $p = \lockinOwnP$) because it is frozen. Note what this
means for the run-level averages above: they cannot distinguish an institution that is stable because
nothing has happened from one that is stable because it cannot move. \emph{This break-alignment was
not pre-registered and is reported as exploratory}; the confirmatory claim is the churn contrast.}
\label{fig:lockin-onset}
\end{figure}

The design implication is more specific than ``memory hurts'', and more usable. What an institution
must not do is consult its own back-catalogue when the question is whether the standing rule still
works. Consulting a comparable authority's back-catalogue costs nothing and---on the exploratory
break-alignment---is the only configuration we measured that is stable while the world is stable and
revises when the world breaks. That is the behaviour the responsiveness literature asks for, and it
is obtained here by changing the \emph{author} of the precedent rather than the amount of it.
