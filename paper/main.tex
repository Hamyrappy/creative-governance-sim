...
lock-in is not caused by how recall is framed, ordered, or scored. No \emph{presentation} of the
agent's own prior decisions that we could construct restored revision. What that leaves is broader
than ``recall'', and the next subsection narrows it: the three repairs share a retrieval rule as well
as a bank, and it turns out to be the retrieval rule that binds.
...
revise across a structural break cannot get there by rewording the precedent. Two things did work.
The $\text{outcome} \times \text{memory}$ interaction restores churn by \emph{adding a channel}
rather than by editing memory; and, as the next subsection shows, so does changing which episodes
retrieval returns. We had written here that the harness must withhold the agent's own past actions.
The fourth arm says something narrower and more useful, so that sentence has been replaced rather
than left standing with a caveat.
...
Our result is not that, and the distinction matters for what one would fix. Retrieval here is
working by its own lights: post-break the component surfaces recent precedent in preference to stale
precedent, at a rate better than chance. The harm survives good retrieval. It is carried by the
retrieval \emph{rule}---rank by similarity to the current state---which in a recurrent environment
returns the agent's immediately preceding decision at precisely the moment the question is whether
that decision still works. A retrieval-quality account cannot reach this, because nothing here is low
quality; the nearest episode really is the most similar one.

The closest statement in the literature is \citet{xu2026memo}, who argue that contextual agentic
memory implements lookup rather than memory, and that because retrieval generalizes by similarity to
stored cases, retrieval-only agents face a ceiling on compositionally novel tasks that no amount of
retrieval quality removes. That is a claim about \emph{capability}---what such an agent cannot
learn to do. Ours is a claim about \emph{behaviour} under the same property: similarity retrieval
over one's own history does not merely fail to generalize, it actively suppresses revision, and it
does so hardest in exactly the stable-then-broken environments where revision is what one wants. We
checked their paper for this claim and it is not made there. We also are not aware of a reported
interaction of the kind in the table above, in which a second, independent channel restores most of
what a memory component removed.

Two levers follow, and we have evidence for both: add an evaluative channel, or change what
retrieval returns. We have tested one instance of the second (episodes from a different agent) and
name the more direct instance (retrieving the agent's own \emph{least} similar precedent) as
specified and not yet run, rather than assert a design principle from a single arm.
