...
\item \textbf{A component whose value reverses between environments}
(\cref{sec:crossworld}). Running the same factorial in a second world, realized-outcome feedback
recovers $975$ loss units on $20/20$ seeds in one and is the harmful channel in the other---both
significant, opposite in sign. A component's value is therefore not a property of the component but
of the match between the failure it can reveal and the failure the world produces. That supplies a
mechanism for the reported poor generalization of automatically evolved harnesses, and a diagnostic
that predicts the sign before the sweep is run.
...
a result about harness research rather than about either world.

\begin{center}\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
world & no harness & outcome feedback & $\Delta$ & seeds helped \\
\midrule
monetary & $3890.53$ & $2915.74$ & $\mathbf{-974.79}$ $[-1218, -739]$ & $\mathbf{20/20}$ \\
epidemic & $24.97$ & $25.75$ & $+0.78$ $[+0.11, +1.38]$ & $5/20$ \\
\bottomrule
\end{tabular}
\end{center}
...
The mechanism is visible in what the agent does with the instrument. The monetary failure is a
refusal to put down a lever that has stopped working: the no-harness arm holds the post-break rate at
$9.51$ where the clairvoyant sits at $1.53$. Outcome feedback is precisely the signal that the
payment is not buying anything---``the policy you have in force scored worse than the one before
it''---and the arm that receives it brings the rate down to $6.99$. In the epidemic the failure is
not that shape at all; there the harmful component is memory, and the failure is lock-in, on which a
report of declining scores offers no purchase.

So the component's value is not a property of the component. It is a property of the match between
the failure mode a channel can reveal and the failure mode the world actually produces. That has a
direct consequence for the practice this paper is about: \emph{a harness tuned on one environment
carries no guarantee into another, and can carry a liability}. \citet{wang2026rethinking} report
exactly this empirically---that automatically evolved harnesses generalize poorly and do not reliably
beat budget-matched test-time scaling---and our measurement supplies a mechanism for it, together
with the diagnostic that predicts the sign in advance.
