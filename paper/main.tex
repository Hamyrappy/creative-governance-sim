...
\emph{Why} it bites harder in one world than the other we do not know, and we would rather leave that
open than furnish a plausible story. The obvious candidate is state recurrence: an endemic epidemic
settles toward an equilibrium, so successive decisions are taken in near-identical states, retrieval
returns near-identical precedent, and repeating it exactly is easy; a monetary economy under a
demand process keeps moving. Measured as the mean nearest-neighbour distance between decision states,
normalized per world, the epidemic is indeed the more recurrent ($0.372$ against $0.452$). But that
is a $20\%$ difference in the proposed mediator against a ninefold difference in the outcome, from
$n=2$ worlds, which is not evidence for a mediator---it is a hypothesis with a direction. Settling it
needs worlds chosen to vary recurrence while holding the rest fixed, which is a study we have not
run.
