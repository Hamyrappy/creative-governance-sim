...
\texttt{r\_star}; the paired monetary arm contains $25{,}300$ occurrences of \texttt{inflation} alone.
...
$9.51$ to $9.12$ ($\Delta = -0.39$, CI $[-0.94, +0.14]$) against an $8.05$-point gap to the
clairvoyant's $1.47$; saturation at the $12.0$ ceiling is $22.7\%$ of post-break steps against
...
\emph{The cancellation story fails on its own window.} One could argue that a prior helps before the
break and hurts after, so the totals cancel. The states entering the break are statistically
indistinguishable between arms (output gap $\Delta = +0.19$, CI $[-0.29, +0.89]$), so the post-break
window is a clean contrast---and on it the disguise buys only $-89$, CI $[-299, +122]$, $12/20$ seeds.
The rescue predicts an effect exactly there and does not get one. Its direction is wrong too: if a
central-banking prior were pinning the feedback gain above the optimal zero, stripping the domain
should push the gain \emph{toward} zero. It pushes it from $1.98$ to $6.69$.

So the correct claim is more general than the one we set out to make, and better supported. The
regent's failure is not that it knows what central banks do. It is that it will not withdraw an
instrument that has stopped working, whatever the instrument is called---it optimizes the objective
term it can see moving, and treats the price as a tax to be paid rather than a reason to stop. That
survives the deletion of every domain cue, and it is invariant across $40$ runs.

\begin{takeaway}
We pre-registered a plausible explanation, built the control that could refute it, and it did. What
replaces it is stronger: the failure is not domain-specific fluency but a domain-general refusal to
abandon a priced lever, and the manipulation check---template removed, failure unmoved---is what
