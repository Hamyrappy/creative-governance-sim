...
essentially nothing for adaptation to do. When the \emph{instrument} changes---the same lockdown now
delivers a quarter of the behavioural response while costing what it always cost---the pre-shock
rule carries a large excess burden, and its feedback structure makes matters worse rather than
better: rising prevalence causes it to intervene harder, buying less each time. The optimal response
is to withdraw from the failing instrument, which is the opposite of what the rule does.
...
We measure a case where these separate completely. Under \emph{total} instrument failure the stale
rule costs $1.51\times$, and the adaptation headroom is exactly $1.00\times$: the best standing rule
is the same before and after the break. Nothing needed to change; the wrong rule had been written.
An authority evaluated in that regime would appear rigid while in fact facing no adaptive problem at
all, and the reform it would be prescribed---greater responsiveness, faster review cycles, better
monitoring---would be the wrong reform.
...
clairvoyant institution has withdrawn; the language-model authority converges on a proportional
response that approximates withdrawal while retaining a small standing intervention.}
