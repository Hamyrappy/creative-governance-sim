...
\section{An authority that knows the textbook applies the textbook}
...
it required a domain about which our authority already had firm views.

Recall the structure: the transmission of the policy rate to the economy collapses at the break, so a
point of interest rate now buys a fraction of the behavioural response it used to buy, while costing
what it always cost. The clairvoyant institution responds by cutting to near the zero lower bound and
holding there. The incumbent Taylor rule, which leans against inflation, instead sees prices drifting
and raises rates---the feedback structure turning protective into harmful, exactly as
\cref{sec:channels-social} describes.

Our language-model authority did something worse than the incumbent rule. It set an average
post-break rate of $9.51$, against the frozen rule's $7.40$ and the clairvoyant's $1.53$, and ended
with a total loss of $3891$ against the clairvoyant's $1871$---worse than every scripted comparison
in the study, including an authority that holds the neutral rate and never acts at all.

The revealing number is not the loss but its decomposition. This authority achieved the \emph{best
inflation and output stabilization of any arm in the experiment}. On the mandate's first objective it
outperformed the clairvoyant institution. It simply spent three times as much doing so, holding a
restrictive stance long after restriction had stopped restricting anything, and the mandate charged
it for every point.

Nor was it confused about the rules it was operating under. Its instructions stated, at every review,
that the rate's cost accrues on the level set and ``is charged whether or not the rate reaches the
economy: a restrictive stance is politically and financially expensive even when it changes nobody's
behaviour.'' That sentence describes the trap precisely. The authority read it, and leaned harder.

What it wrote was not confused either. Across four hundred decisions it composed $123$ distinct
rules, nearly all recognizable Taylor rules with conventional coefficients. The calibrated optimum in
this world sets the feedback coefficient to \emph{zero}. Our authority was not governing badly in the
sense of governing randomly; it was governing by the book, in a world where the book had stopped
applying.

We think this is the most policy-relevant observation in the paper, and it cuts against an intuition
that is easy to hold. One would expect an authority with deep familiarity in a policy domain to
govern it better. Here familiarity is what produced the failure: a prior about what central banks do,
strong enough to survive an explicit written instruction designed to displace it. The epidemic could
not have revealed this, because no comparable canonical answer exists for setting a lockdown
threshold. It takes a domain with a textbook to show a textbook being wrong.

\begin{takeaway}
The failure was fluency, not incompetence. An authority that knows the standard response will supply
the standard response, and telling it in writing that the standard response no longer works was not
enough to stop it.
\end{takeaway}
