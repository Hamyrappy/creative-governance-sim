...
| Phase | What | Status |
|---|---|---|
| 0 | thin core + data spine (6 seams, LLM cache/replay, scalar plugin, cubic migration, Runner, ResultStore, CLI, golden-master, guard tests) | **DONE** (this branch) |
| 1 | `LLMRegent` + prompt assembler + PID/LQR/OPRO baselines; `StabilizationLoss`; coupled system | LLMRegent + PID/LQR landed; OPRO + coupled pending |
| 2 | rollout-free harness (`TraceFeedback`, `EpisodicMemory`); then `RolloutProbe`; H1 result | components landed; the H1 *result* needs a live model run (author) |
